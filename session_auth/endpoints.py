...
class SessionLoginEndpoint(HTTPEndpoint, metaclass=ABCMeta):
    ...
    def _render_template(
        ...
        csrftoken = request.scope.get("csrftoken")
        csrf_cookie_name = request.scope.get("csrf_cookie_name")
        # check if ?nextURL set in query_params
        login_nextURL = request.query_params.get('nextURL')
 
        return HTMLResponse(
            self._login_template.render(
                csrftoken=csrftoken,
                csrf_cookie_name=csrf_cookie_name,
                login_nextURL=login_nextURL,
                request=request,
    ...
    async def post(self, request: Request) -> Response:
        ...
        nextURL = body.get('nextURL')       # check if nextURL is hidden in form
        if nextURL:
            response: Response = RedirectResponse(
                url=nextURL, status_code=HTTP_303_SEE_OTHER
            )
